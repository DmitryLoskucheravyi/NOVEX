document.addEventListener("DOMContentLoaded", () => {
  ...
  const elements = {
    ...
    coinNameSpan: document.querySelectorAll(selectors.coinNameSpan),
    networkNameOutput: document.querySelectorAll(selectors.networkNameOutput),
  ...
  function updateDepositDetails(networkCard) {
    ...
    if (elements.networkNameOutput) {
      elements.networkNameOutput.forEach((el) => { el.textContent = network });
    }
    ...
    if (elements.coinNameSpan) {
      elements.coinNameSpan.forEach((el) => {
        el.textContent = state.currentCoin;
      })
    }
  }
  ...
  async function pasteToAddressInput() {
    console.log('1')
    console.log(elements.copyBtn, elements.depositAddress)
    if (!elements.copyBtn || !elements.depositAddress) return;


    try {
      const text = await navigator.clipboard.readText();
      elements.depositAddress.value = text;
      ...
      console.error("Не вдалося вставити адресу", error);
    }
  }
  ...
  function bindEvents() {
    ...
    if (elements.copyBtn) {
      elements.copyBtn.addEventListener("click", pasteToAddressInput);
    }
